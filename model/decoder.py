...
class Decoder(nn.Module):
    ...
    def __init__(self, inchannel=cfg.para_maps, outchannel=cfg.input_dim, channels_list=[4, 8, 16, 32, 32, 64, 128, 256, 512]):
        ...
        self.down1 = down_block(pool_size=2, pool_stride=2, inchannel=channels_list[0], outchannel=channels_list[1],
                                skip_channel=channels_list[7], conv_group=2, basic_1_num=2)  # [batch_size, channels_list[1], 110, 110]
        self.down2 = down_block(pool_size=2, pool_stride=2, inchannel=channels_list[1], outchannel=channels_list[2],
                                skip_channel=channels_list[6], conv_group=2, basic_1_num=2)  # [batch_size, channels_list[2], 55, 55]
        self.down3 = down_block(pool_size=5, pool_stride=2, inchannel=channels_list[2], outchannel=channels_list[3],
                                skip_channel=channels_list[5], conv_group=2, basic_1_num=2)  # [batch_size, channels_list[3], 26, 26]
        self.down4 = down_block(pool_size=2, pool_stride=2, inchannel=channels_list[3], outchannel=channels_list[4],
                                skip_channel=channels_list[4], conv_group=2, basic_1_num=2)  # [batch_size, channels_list[4], 13, 13]
        self.basic_block_1 = nn.Sequential(*[
            BasicBlock(inplanes=channels_list[4], planes=channels_list[4], stride=1, groups=2, downsample=None) for _ in range(2)])
        ...
        self.up1 = up_block(up_size=(26, 26), inchannel=2*channels_list[4], outchannel=channels_list[5],
                            conv_group=2, basic_1_num=2)  # [batch_size, channels_list[3], 26, 26]
        self.up2 = up_block(up_size=(55, 55), inchannel=2*channels_list[5], outchannel=channels_list[6],
                            conv_group=2, basic_1_num=2)  # [batch_size, channels_list[2], 55, 55]
        self.up3 = up_block(up_size=(110, 110), inchannel=2*channels_list[6], outchannel=channels_list[7],
                            conv_group=2, basic_1_num=2)  # [batch_size, channels_list[1], 110, 110]
        self.up4 = up_block(up_size=(220, 220), inchannel=2*channels_list[7], outchannel=channels_list[8],
                            conv_group=2, basic_1_num=2)  # [batch_size, channels_list[0], 220, 220]
        self.tail = nn.Sequential(conv3x3(in_planes=channels_list[8], out_planes=outchannel, stride=1, groups=2),
                                  nn.Tanh()
